...
class PostListView(ListView):
    ...
    def get_queryset(self):
        queryset = (
            ...
            .select_related("author", "category", "product_category")
    ...
    def get_context_data(self, **kwargs):
        ...
        context["navbar_items"] = NavMenu.objects.filter(is_active=True).order_by(
            "order"
        )
        return context
...
class PostDetailView(DetailView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            return (
                Post.objects.filter(Q(status="published") | Q(author=self.request.user))
                .select_related("author", "category", "product_category")
                .prefetch_related("tags", "comments", "related_products")
            )
        return (
            Post.objects.filter(status="published")
            .select_related("author", "category", "product_category")
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context["is_liked"] = self.object.likes.filter(
...
class TagDetailView(ListView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(status="published", tags=self.tag).select_related(
            "author", "category"
        )
...
class UserPostListView(ListView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(author=self.user, status="published").select_related(
            "category"
        )
