...
class PaymentForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if not instance.payment_number:
            ...
            now = timezone.now()
            # Include date and time for uniqueness (YYYYMMDD-HHMMSS format)
            datetime_str = now.strftime("%Y%m%d-%H%M%S")
            # Add microseconds for extra uniqueness in case of concurrent requests
            microseconds = now.strftime("%f")[:3]  # First 3 digits of microseconds
            instance.payment_number = f"PAY-{datetime_str}-{microseconds}"
...
class InvoiceShipmentForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "ship_date": forms.DateTimeInput(
                attrs={"class": "input input-bordered w-full", "type": "datetime-local"}
            ),
            "estimated_delivery": forms.DateTimeInput(
                attrs={"class": "input input-bordered w-full", "type": "datetime-local"}
            ),
            "actual_delivery": forms.DateTimeInput(
                attrs={"class": "input input-bordered w-full", "type": "datetime-local"}
            ),
            "delivered_to": forms.TextInput(
                attrs={
                    "class": "input input-bordered w-full",
                    "placeholder": "Name of person who received",
                }
            ),
            "delivery_signature": forms.Textarea(
                attrs={
            ...
            "ship_to_company": forms.TextInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            "ship_to_address_line1": forms.TextInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            "ship_to_address_line2": forms.TextInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            ...
            "ship_to_postal_code": forms.TextInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            "ship_to_country": forms.TextInput(
                attrs={"class": "input input-bordered w-full", "maxlength": "2"}
            ),
            "ship_to_phone": forms.TextInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            "ship_to_email": forms.EmailInput(
                attrs={"class": "input input-bordered w-full"}
            ),
            "internal_notes": forms.Textarea(
                attrs={
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk and invoice:
            if invoice.shipping_same_as_billing or not invoice.shipping_address_line1:
                # Use billing address
                self.initial["ship_to_name"] = (
                    f"{invoice.first_name} {invoice.last_name}".strip()
                )
                self.initial["ship_to_company"] = (
...
class ShipmentItemForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "invoice_item": forms.Select(
                attrs={"class": "select select-bordered w-full"}
            ),
            "quantity_shipped": forms.NumberInput(
                attrs={
                    "class": "input input-bordered w-full",
                    "step": "0.01",
                    "min": "0.01",
